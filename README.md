...
*token*                     |*记号*         | [lex.token]   | *标识符* \| *关键词* \| *字面量* \| *头文件名* \| *运算符或标点*
...
Original   |中文   |章节    |定义
|-|-|-|-|
*splice-specifier*          |*拼接说明符*   | [basic.splice]  | `[:` *常量表达式* `:]`
*splice-specialization-specifier*|*拼接说明符*| [basic.splice]| *拼接说明符* `<` *模板实参列表*__?__ `>`
...
*compound-requirement*      |*复合规定*     | [expr.prim.req.compound] | `{` *表达式* `}` *noexcept-说明符*__?__ *返回类型规定*__?__ `;`
...
*unary-expression*          |*一元表达式*   | [expr.unary.general] | *后缀表达式* \|<br> (*一元运算符*\|`++`\|`--`) *转型表达式* \|<br> *等待表达式* \|<br> `sizeof` (*一元表达式* \| `(` *无函数类型标识* `)` \| `...` `(` *标识符* `)`) \|<br> `alignof` `(` *无函数类型标识* `)` \|<br> *noexcept-表达式* \|<br> *new-表达式* \|<br> *delete-表达式* \|<br> *反射表达式*
...
*cast-expression*           |*转型表达式*   | [expr.cast]   | ( `(` *无函数类型标识* `)` )__\*__ *一元表达式*
...
*expansion-statement*       |*展开语句*     | [stmt.expand] | `template` `for` `(` *初始化语句*__?__ *for-范围声明式* `:` *展开初始化式* `)` *复合语句*
*expansion-initializer*     |*展开初始化式* | [stmt.expand] | *表达式* \| `(` *表达式列表*__?__ `)`
*expansion-init-list*       |*展开初始化列表* | [stmt.expand] | `{` *表达式列表* `,`__?__ `}` \| `{` `}`
...
*sb-identifier-list*        |*sb-标识符列表*| [dcl.pre]  | *sb-标识符* (`,` *sb-标识符*)__\*__
*structured-binding-declaration*|*结构化绑定声明式*| [dcl.pre] | *属性说明符序列*__?__ *声明说明符序列* *引用限定符*__?__ `[` *sb-标识符列表* `]`
...
*simple-type-specifier*     |*简单类型说明符*| [dcl.type.simple] | *嵌套名说明符*__?__ (*类型名* \| *简单模板名*) \|<br> *嵌套名说明符* `template` *简单模板标识* \|<br> *计算类型说明符* \| *占位符类型说明符* \| *包组索引模板名* \|<br> `char` \| `char8_t` \| `char16_t` \| `char32_t` \| `wchar_t` \|<br> `bool` \| `short` \| `int` \| `long` \|<br> `signed` \| `unsigned` \| `float` \| `double` \| `void`
...
*trailing-returntype*       |*尾部返回类型* | [dcl.decl.general] | `->` *无函数类型标识*
...
*nofun-type-id*             |*无函数类型标识* | [dcl.name]    | *类型说明符序列* *无函数声明符*__?__
*nofun-declarator*          |*无函数声明符*   | [dcl.name]    | *有指针无函数声明符* \| *无指针无函数声明符* *形参和限定符* *尾部返回类型*
*ptr-nofun-declarator*   |*有指针无函数声明符*| [dcl.name]   | *指针运算符*__\*__ *无指针无函数声明符*
*noptr-nofun-declarator* |*无指针无函数声明符*| [dcl.name] | ( `(` *有指针无函数声明符* `)` )__?__ ( ∅ \| *形参和限定符* \| (`[` *常量表达式*__?__ `]` *属性说明符序列*__?__)__\+__ )
*type-id*                   |*类型标识*     | [dcl.name]    | *类型说明符序列* *抽象声明符*__?__
*defining-type-id*          |*定义类型标识* | [dcl.name]    | *定义类型说明符序列* *抽象声明符*__?__
*abstract-declarator*       |*抽象声明符*   | [dcl.name]    | *有指针抽象声明符* \| *无指针抽象声明符*__?__ *形参和限定符* *尾部返回类型* \| *抽象包组声明符*
*ptr-abstract-declarator*   |*有指针抽象声明符*| [dcl.name]   | *指针运算符*__\*__ *无指针抽象声明符*
*noptr-abstract-declarator* |*无指针抽象声明符*| [dcl.name] | ( `(` *有指针抽象声明符* `)` )__?__ ( ∅ \| *形参和限定符* \| (`[` *常量表达式*__?__ `]` *属性说明符序列*__?__)__\+__ )
*abstract-pack-declarator*  |*抽象包组声明符*| [dcl.name]   | *指针运算符*__\*__ *无指针抽象包组声明符*
*noptr-abstract-pack-declarator*|*无指针抽象包组声明符*| [dcl.name] | `...` ( ∅ \| *形参和限定符* )__\+__ )
...
*designated-only-initializer-list*|*仅定名初始化式列表*| [dcl.init.general] | *定名初始化式子句* ( `,` *定名初始化式子句* )__\*__
*designated-initializer-list*|*定名初始化式列表*| [dcl.init.general] | (*初始化式列表* `,`)__?__ *仅定名初始化式列表*
...
*alignment-specifier*       |*对齐说明符*   | [dcl.attr.grammar] | `alignas` `(` (*无函数类型标识* \| *常量表达式*) `...`__?__ `)`
...
*class-or-decltype*         |*类或-decltype*| [class.derived.general] | *嵌套名说明符*__?__ *类型名* \|<br> *嵌套名说明符* `template` *简单模板标识* \|<br> *计算类型说明符*
...
*template-parameter*        |*模板形参*     | [temp.param]  | *类型形参* \| *形参声明式* \| *类型-tt-形参* \| *变量-tt-形参* \| *概念-tt-形参*
*type-parameter*            |*类型形参*     | [temp.param]  | *类型形参关键词* ( `...`__?__ *标识符*__?__ \| *标识符*__?__ `=` *类型标识* ) \|<br> *类型约束* ( `...`__?__ *标识符*__?__ \| *标识符*__?__ `=` *类型标识* )
*type-parameter-key*        |*类型形参关键词*| [temp.param] | `class` \| `typename`
*type-constraint*           |*类型约束*     | [temp.param]  | *嵌套名说明符*__?__ *概念名* ( `<` *模板实参列表* `>` )__?__
*type-tt-parameter*         |*类型-tt-形参* | [temp.param]  | *模板头* *类型形参关键词* ( `...`__?__ *标识符*__?__ \| *标识符*__?__ *类型-tt-形参默认实参* )
*type-tt-parameter-defualt* |*类型-tt-形参默认实参* | [temp.param] | `=` ( *嵌套名说明符*__?__ \| *嵌套名说明符* `template` ) *模板名*
*variable-tt-parameter*     |*变量-tt-形参* | [temp.param]  | *模板头* `auto` ( `...`__?__ *标识符*__?__ \| *标识符*__?__ `=` *嵌套名说明符*__?__ *模板名* )
*concept-tt-parameter*      |*概念-tt-形参* | [temp.param]  | `template` `<` *模板形参列表* `>` `concept` ( `...`__?__ *标识符*__?__ \| *标识符*__?__ `=` *嵌套名说明符*__?__ *模板名* )
*simple-template-id*        |*简单模板标识* | [temp.names]  | *模板名* `<` *模板实参列表*__?__ `>`
*template-id*               |*模板标识*     | [temp.names]  | *简单模板标识* \|<br> ( *运算符函数标识* \| *字面量运算符标识* ) `<` *模板实参列表*__?__ `>` 
*template-name*             |*模板名*       | [temp.names]  | *标识符* ( `...` `[` 常量表达式 `]` )__?__
*pack-index-template-name*  |*包组索引模板名* | [temp.names]  | *标识符* `...` `[` 常量表达式 `]`
*simple-template-name*      |*简单模板名*   | [temp.names]  | *标识符*
*template-argument-list*    |*模板实参列表* | [temp.names]  | *模板实参* `...`__?__ ( `,` *模板实参* `...`__?__ )__\*__
*template-argument*         |*模板实参*     | [temp.names]  | *模板实参名* \| *常量表达式* \| *类型标识* \| *花括号初始化列表*
*template-argument-name*    |*模板实参名*   | [temp.names]  | ( *嵌套名说明符*__?__ \| *嵌套名说明符* `template` ) *标识符*
*constraint-expression*     |*约束表达式*   | [temp.constr.decl] | *逻辑或表达式*
*deduction-guide*           |*推断导引*     | [temp.deduct.guide] | *explicit-说明符*__?__ *简单模板名* `(` *形参声明子句* `)` `->` *简单模板标识* `;`
...
*module-file*               |*模块文件*     | [cpp.pre]     | *行指令序列*__?__ *预处理全局模块分段*__?__ *预处理模块* *组*__?__ *预处理私有模块分段*__?__
...
*control-line*              |*控制行*       | [cpp.pre]     | `#` `include` *预处理记号序列* *换行* \|<br> *预处理导入* \|<br> `#` `embed` *预处理记号序列* *换行* \|<br> `#` `define` *标识符* ( ∅ \| *左括号* ( ∅ \| `...`__?__ \| *标识符列表* ( `,` `...` )__?__ ) `)` ) *替换列表* *换行* \|<br> `#` `undef` *标识符* *换行* \|<br> *行指令* \|<br> `#` `error` *预处理记号序列*__?__ *换行* \|<br> `#` `warning` *预处理记号序列*__?__ *换行* \|<br> `#` `pragma` *预处理记号序列*__?__ *换行* \|<br> `#` *换行*
*line-directives*           |*行指令序列*   | [cpp.pre]     | *行指令*__\+__
...
*header-name-tokens*        |*头文件名记号序列*| [cpp.cond] | *普通字符串字面量* \| `<` *h-预处理记号序列* `>`
...
bracket resource search                 |尖括号资源搜索 |`#embed` 的 `<>` 搜索
...
canonical index type                    |规范索引类型   |`mdspan`
canonical slice type                    |规范分片类型   |`mdspan`
...
collapsing slice type                   |折叠分片类型 | `mdspan`
...
compatible of template argument/parameter |模板实参/形参兼容
...
concept-dependent constraint            |概念待决约束|含有概念模板形参的约束
concept-id                              |概念标识   |概念的*简单模板标识*，`bool`纯右值，规范化约束表达式的值
concept template parameter              |概念模板形参|接受概念为实参
concurrency                             |并发性
concurrent                              |并发的
...
constant template parameter             |常量模板形参   |接受常量对象为实参
...
constexpr-enabled                       |启用 constexpr 的  |`formatter` 特例
...
constexpr-unknown representation        |constexpr 未知表示  |
consteval-only                          |仅 consteval    |函数或变量只能在编译期常量求值中使用
...
corresponding direct subobject          |对应直接子对象
...
debug-enabled                           |启用调试的 |`formatter` 特例
decay                                   |退化
...
destructuring expansion statement       |解构展开语句|`template for（auto x:obj)`
...
eligible for subsumption                |有纳入资格 |非概念待决约束
...
enumerating expansion statement         |列举展开语句|`template for（auto x:{a, b, c})`
...
expansion-iterable                      |可展开迭代     |展开语句的目标
expansion statement                     |展开语句       |列举，迭代，解构
explicit                                |显式，明确
...
fold expanded constraint                |折叠展开约束
fold expression                         |折叠表达式
follow program point                    |跟随程序位置点
...
source search                           |头文件搜索 |`#include` 的 `<>` 搜索
...
immediate constant expression           |直接常量表达式
...
implementation-resource-width           |实现资源宽度   |嵌入资源的位数
implicit                                |隐式，暗中，隐含
...
impose                                  |施加
inactive union elemental subobject      |非活跃联合体元素子对象
...
iterating expansion statement           |迭代展开语句|`template for（auto x:cont)`
...
potentially usable in constant expression|潜在可被用在常量表达式
...
quote resource search                   |引号资源搜索   |`#embed` 的 `"` 搜索
...
range adaptor closure object            |范围适配器闭包对象
range adaptor object                    |范围适配器对象
...
sliceable layout mapping requirements   |可分片布局映射规定
slice range                             |分片范围       |`mdspan`
slice type                              |分片类型       |`mdspan`
source character set                    |源字符集
source file                             |源文件
source file inclusion                   |源文件包含 |`#include <h>`或`#include "s"`或`#include M`。支持宏替换，没有字符串拼接
source file search                      |源文件搜索 |`#include` 的 `"` 搜索
...
template template parameter             |模板模板形参   |三种模板形参之一。引入模板名，有类型模板、变量模板和概念三种
...
type template parameter                 |类型模板形参   |接受类型为实参
type template template parameter        |类型模板模板形参|接受类模板、别名模板为实参
...
union elemental subobject               |联合体元素子对象
...
usable in constant expression           |可被用在常量表达式
...
variable template template parameter    |变量模板模板形参|接受变量模板为实参
